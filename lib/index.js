...
class Payment {
  constructor({ appid, mchid, partnerKey, pfx, notify_url, refund_url, spbill_create_ip, sandbox } = {}, debug = false) {
    ...
    this.urls = sandbox ? sandboxUrl : urls;
  ...
  static async sandbox(config, debug) {
    const { sandbox_signkey } = await Payment.init(config).getSignkey();
    return new Payment({
      ...config,
  ...
  async _request(params, type, cert = false) {
    ...
    const { status, data } = await urllib.request(this.urls[type], pkg);
  ...
  getNativeUrl(params) {
    ...
    const url = 'weixin://wxpay/bizpayurl'
  ...
  async downloadBill(params, format = false) {
    ...
    const xml = await this._request(pkg, 'downloadbill');
  ...
  async downloadFundflow(params, format = false) {
    ...
    const xml = await this._request(pkg, 'downloadfundflow', true);
    return this._parseBill(xml, format);
  }
